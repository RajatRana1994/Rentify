...
class IntRateOfReturn(
    ...
    init {
        (context as ChartFlowActivity).apply {
            ...
            btnNext.setOnSingleClickListener { listener.triggerNext(ChartFlowActivity.TaxBenefitSchedule_TYPE) }
            listener.prepareNext(ChartFlowActivity.TaxBenefitSchedule_TYPE)
        }
    }
    ...
    private fun setChartData() {
        (context as ChartFlowActivity).apply {
            ...
            val temp=
                ChartFlowActivity.cashReturnV.div(ChartFlowActivity.cashReqV).times(100)+ ChartFlowActivity.principalPaymentV.div(
                    RentifyApp.inputDataEntity.Purchase_Price!!).times(100)+ RentifyApp.inputDataEntity.Area_Appreciation_Growth!!.div(4)+ RentifyApp.inputDataEntity.Annual_Rental_Increase!!.times(0.37)
            val temp1=(ChartFlowActivity.structureV.div(39).plus(ChartFlowActivity.upratingV)).div(
                RentifyApp.inputDataEntity.Purchase_Price!!).times(100)

            total=temp+temp1
            chart1.setCenterTextTypeface(Typeface.DEFAULT_BOLD)
            chart1.centerText = """IRR
...
//            }
            ...
            rvLegends.adapter = LegendAdapter(legendsList,false)
