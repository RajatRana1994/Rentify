...
class FilesActivity : AppCompatActivity(), CheckPermission.PermissionListener,
    ...
    private fun getLocalFiles() {
        ...
        if (appFolderPath()!!.exists()) {
            val files = appFolderPath()!!.listFiles()
            files.forEach {
    ...
    private fun showFiles(file: File) {
        ...
        view.findViewById<ImageView>(R.id.ivMove).setOnSingleClickListener {
            if (llFolderInflater.childCount > 0) {
                val mBuilder = AlertDialog.Builder(this@FilesActivity)
                ...
                mBuilder.setSingleChoiceItems(
                    ...
                    moveFile(
                        file,
                        File(appFolderPath()!!.path + File.separator + listItems[which] + File.separator + file.name)
                    )
                }
    ...
    private fun checkToEditPdf(file: File) {
        session(this@FilesActivity).pref.all.entries.forEach {
...
//                }
                startActivity(Intent(this@FilesActivity,InputActivity::class.java).putExtra("pdf_data",it.value!! as String))
            }
        }
    }
    ...
    private fun showFolders(folder: File) {
        ...
        view.findViewById<ImageView>(R.id.ivFolder).setOnSingleClickListener {
            startActivity(Intent(this@FilesActivity,SubFilesActivity::class.java).apply {
    ...
    private fun createFolder(foldername: String) {
        if (File(appFolderPath()!!.path + File.separator + foldername).exists().not()) {
            val created =
                File(appFolderPath()!!.path + File.separator + foldername)!!.mkdir()
            Toast.makeText(
                this@FilesActivity,
            ...
            getLocalFiles()
        } else Toast.makeText(this@FilesActivity, "Already Exist!", Toast.LENGTH_SHORT).show()
    }
...
//                    chooserIntent.putExtra(Intent.EXTRA_INITIAL_INTENTS, shareIntent(arrayOfNulls<Parcelable>(shareIntent.size())))
                    ...
                    Toast.makeText(this@FilesActivity, e.toString(), Toast.LENGTH_LONG).show()
                }
            }
        } catch (e: Exception) {
            try {
